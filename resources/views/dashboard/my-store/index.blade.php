        ...
        <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
            <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
                <div class="w-full md:w-1/2">
                    <form action="{{ route('dashboard.my-store.index') }}" method="GET" class="flex items-center">
                ...
                <div class="w-full md:w-auto flex flex-col md:flex-row md:items-center justify-end !ml-0">
                    <a href="{{ route('dashboard.my-store.create') }}"
                ...
                <table class="w-full text-sm text-right text-gray-500 dark:text-gray-400">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                        <tr>
                            ...
                            <th scope="col" class="px-4 py-3">مُـدخل النظام</th>
                            @endif
                            <th scope="col" class="px-4 py-3">عنوان النظام</th>
                            <th scope="col" class="px-4 py-3">السعر</th>
                            <th scope="col" class="px-4 py-3">الصورة</th>
                            <th scope="col" class="px-4 py-3">مدة الدعم الفني</th>
                        ...
                        <tr class="border-b dark:border-gray-700">
                            ...
                            <td class="px-4 py-3">
                                ...
                                <span class="text-xs bg-green-700 text-green-200 rounded-xl px-1">{{
                                    $system->onwer_system }}</span>
                                @endif
                                {{ Str::limit($system->name_ar, 20) }}
                            </td>
                            <td class="px-4 py-3 flex items-center">
                                <div class="flex items-center gap-1">
                                    {{ number_format($system->original_price) }}
                                    <x-drhm-icon width="12" height="14" />
                                </div>
                            </td>
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-1 text-[12px]">
                                    <img src="{{ asset($system->main_image) }}" alt="dollar" class="w-10 h-10 rounded">
                                </div>
                            </td>
                            ...
                            <td class="px-4 py-3 flex items-center justify-end">
                                <div class="flex justify-center gap-2">
                                    <a href="{{ route('dashboard.my-store.show', $system) }}"
                                    ...
                                    {{-- زر الحذف --}}
                                    <form action="{{ route('dashboard.my-store.destroy', $system->id) }}" method="POST"
                                        class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
