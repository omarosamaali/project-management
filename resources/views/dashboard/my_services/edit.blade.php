        ...
        <div class="p-3 bg-white dark:bg-gray-800 relative shadow-xl border rounded-xl overflow-hidden">
            ...
            <form method="POST" action="{{ route('dashboard.my_services.update', $myService) }}" class="space-y-6"
                    ...
                    <div
                        ...
                        <div class="flex items-center">
                            <input type="radio" id="service_{{ $service->id }}" name="service_id"
                                value="{{ $service->id }}"
                                {{ $myService->service_id == $service->id ? 'checked' : '' }}
                ...
                <div>
                    ...
                    <input value="{{ old('price', $myService->price) }}" type="number" id="price" name="price" placeholder="ادخل السعر" required
                        class="placeholder-gray-500 w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 transition duration-150 ease-in-out">
                ...
                <div>
                    ...
                    <input value="{{ old('duration', $myService->duration) }}" type="number" id="duration" name="duration" placeholder="مدة التسليم بالأيام مثال 30"
                        required
                        class="placeholder-gray-500 w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 transition duration-150 ease-in-out">
                    @error('duration')
                    <span class="text-red-500 text-xs mt-1">{{ $message }}</span>
                ...
                <div>
                    ...
                    <textarea rows="5" id="description" name="description" placeholder="ادخل تفاصيل عن الخدمة" required
                        class="placeholder-gray-500 w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-blue-500
                                        focus:border-blue-500 transition duration-150 ease-in-out">{{ old('description', $myService->description) }}</textarea>
                    @error('description')
                    <span class="text-red-500 text-xs mt-1">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <textarea rows="5" id="what_you_will_get" name="what_you_will_get"
                        placeholder="ادخل ماذا سوف يحصل العميل" required
                        class="placeholder-gray-500 w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-blue-500
                                                                        focus:border-blue-500 transition duration-150 ease-in-out">{{ old('what_you_will_get', $myService->what_you_will_get) }}</textarea>
