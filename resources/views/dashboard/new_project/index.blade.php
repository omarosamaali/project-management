...
@section('title', 'المشاريع الجديدة')

@section('content')

<section class="!pl-0 p-3 sm:p-5">
    <x-breadcrumb first="الرئيسية" link="#" second="المشاريع الجديدة" />
                ...
                <table class="w-full text-sm text-right text-gray-500 dark:text-gray-400">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                        <tr>
                            ...
                            <th scope="col" class="px-4 py-3 whitespace-nowrap">نوع الطلب</th>
                    ...
                    @foreach($specialRequests as $newProject)
                    <tbody>
                        <tr class="border-b dark:border-gray-700">
                            ...
                            <td class="px-4 py-3">
                                {{ Str::limit($newProject->title, 20) }}
                            </td>
                            <td class="px-4 py-3">
                                {{ $newProject->project_type }}
                            </td>
                            <td class="px-4 py-3">
                                {{ Str::limit($newProject->description, 40) }}
                            </td>
                            <td>
                                {{ $newProject->created_at->format('Y-m-d') }}
                            </td>
                            <td class="px-4 py-3 text-sm text-gray-600 whitespace-nowrap">
                                {{ $newProject->status_name }}
                            </td>
                            <td class="px-4 py-3 flex items-center justify-end">
                                <a href="{{ route('dashboard.new_project.show', $newProject) }}"
