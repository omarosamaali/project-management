...
@section('title', 'تعديل طلب خاص')

@section('content')

<section class="p-3 sm:p-5">
    <x-breadcrumb first="الرئيسية" link="{{ route('special-request.show') }}" second="تعديل الطلب" />
</section>
<div class="max-w-4xl mb-5 border mx-auto p-6 lg:p-8 bg-white dark:bg-gray-800 shadow-md sm:rounded-lg">
    <h2 class="text-3xl font-bold text-gray-900 dark:text-white mb-6 text-center">✏️ تعديل الطلب: {{
        $specialRequest->title }}</h2>
    <form action="{{ route('special-request.update', $specialRequest) }}" method="POST">
        @csrf
        @method('PUT') 
        ...
        <div class="mb-8 p-6 border border-gray-200 dark:border-gray-700 rounded-lg">
            ...
            <div class="mb-4">
                ...
                <x-text-input id="title" class="block mt-1 w-full focus:border-red-500 focus:ring-red-500" type="text"
                    name="title" :value="old('title', $specialRequest->title)" required />
            ...
            <div class="mb-4">
                ...
                <select id="project_type" name="project_type"
                    ...
                    required>
                    {{-- التعديل الثالث: اختيار القيمة الحالية --}}
                    @php $currentType = old('project_type', $specialRequest->project_type); @endphp
                    <option value="" class="text-gray-500">-- اختر نوع الطلب --</option>
                    <option value="web" {{ $currentType=='web' ? 'selected' : '' }}>موقع ويب (Web
                        Application)</option>
                    <option value="mobile" {{ $currentType=='mobile' ? 'selected' : '' }}>تطبيق
                        موبايل (Mobile App)</option>
                    <option value="both" {{ $currentType=='both' ? 'selected' : '' }}>كلاهما (Web &
                        Mobile)</option>
                    <option value="logo" {{ $currentType=='logo' ? 'selected' : '' }}>تصميم شعار
                        (Logo)</option>
                    <option value="identity" {{ $currentType=='identity' ? 'selected' : '' }}>تصميم
                        هوية مؤسسة (Business Identity)</option>
                    <option value="digital" {{ $currentType=='digital' ? 'selected' : '' }}>تسويق
                        إلكتروني (Digital Marketing)</option>
                    <option value="management" {{ $currentType=='management' ? 'selected' : '' }}>
                        إدارة موقع او تطبيق (Website Management)</option>
                    <option value="social" {{ $currentType=='social' ? 'selected' : '' }}>إدارة حساب
                        التواصل الاجتماعي (Social Media Management)</option>
                    <option value="training" {{ $currentType=='training' ? 'selected' : '' }}>دورة
                        تدريبية (Training)</option>
                    <option value="consulting" {{ $currentType=='consulting' ? 'selected' : '' }}>
                        استشارات تقنية (Technical Consulting)</option>
                    <option value="other" {{ $currentType=='other' ? 'selected' : '' }}>طلب اخر
                        (Other)</option>
                </select>
            ...
            <div class="mb-4">
                ...
                {{-- التعديل الثاني: تعبئة القيمة الحالية من قاعدة البيانات --}}
                <textarea id="description" name="description" rows="5"
                    ...
                    required>{{ old('description', $specialRequest->description) }}</textarea>
        ...
        <div class="mb-8 p-6 border border-gray-200 dark:border-gray-700 rounded-lg">
            ...
            <div class="mb-4">
                ...
                <textarea id="core_features" name="core_features" rows="4"
                    ...
                    class="placeholder-gray-500 border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-red-500 dark:focus:border-red-600 focus:ring-red-500 dark:focus:ring-red-600 rounded-md shadow-sm block mt-1 w-full"
                    required>{{ old('core_features', $specialRequest->core_features) }}</textarea>
            ...
            <div class="mb-4">
                ...
                <x-text-input id="examples"
                    ...
                    name="examples" :value="old('examples', $specialRequest->examples)"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <x-text-input id="budget"
                        ...
                        type="number" name="budget" :value="old('budget', $specialRequest->budget)"
                ...
                <div>
                    ...
                    <input id="deadline" type="date" name="deadline"
                        value="{{ old('deadline', $specialRequest->deadline) }}"
                        class="placeholder-gray-500 border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-red-500 dark:focus:border-red-600 focus:ring-red-500 dark:focus:ring-red-600 rounded-md shadow-sm block mt-1 w-full">
