    ...
    <div class="mx-auto max-w-4xl">
        ...
        <div class="bg-white shadow-none rounded-xl overflow-hidden border-2 border-black print-container invoice-mono">
            ...
            <div class="p-8">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full border-collapse">
                        ...
                        <tbody class="divide-y divide-gray-200">
                            @php
                            $invoiceAmounts = \App\Support\InstallmentPaymentService::invoiceAmounts($payment ?? null, $installment ?? null);
                            $baseAmount = $invoiceAmounts['base'];
                            $gatewayFees = $invoiceAmounts['fees'];
                            $totalWithFees = $invoiceAmounts['total'];
                            @endphp
                            <tr>
                            ...
                            <tr>
                                <td class="py-2 px-4 text-black text-[10px] leading-tight">رسوم بوابة الدفع <span class="text-[9px] text-gray-700">(7.9% + 2)</span></td>
                                <td class="text-center py-3 px-4 text-black font-bold">
                                    <div class="flex items-center gap-1 justify-center">
                                       <span dir="ltr">{{ number_format($gatewayFees, 2) }}</span>
                                ...
                                <td class="py-2 px-4 text-black text-left text-[10px] leading-tight">Payment Gateway Fees <span class="text-[9px] text-gray-700">(7.9% + 2)</span></td>
                            </tr>
                ...
                @if($installment)
                @php
                    $projectTotal = $specialRequest->budget_total ?? $specialRequest->price;
                    $dueDateLabel = $installment->due_date
                        ? \Carbon\Carbon::parse($installment->due_date)->format('Y/m/d')
                        : null;
                @endphp
                <div class="mt-4 px-4 py-3 bg-white border-2 border-black rounded-lg">
                    <p class="text-xs text-black font-bold mb-2 underline">معلومات القسط / Installment Info:</p>
                    <div class="space-y-1.5 text-xs text-black">
                        <div class="flex justify-between items-start gap-6">
                            <div class="text-right flex-1">• اسم الدفعة: <strong>{{ $installment->payment_name }}</strong></div>
                            <div class="text-left flex-1" dir="ltr">• مبلغ القسط: <strong>{{ number_format($installment->amount, 2) }}</strong></div>
                        </div>
                        <div class="flex justify-between items-start gap-6">
                            <div class="text-right flex-1">• إجمالي المشروع: <strong>{{ number_format($projectTotal, 2) }}</strong></div>
                            <div class="text-left flex-1" dir="ltr">• المتبقي: <strong>{{ number_format($specialRequest->remaining_amount, 2) }}</strong></div>
                        </div>
                        @if($dueDateLabel)
                        <div class="flex justify-between items-start gap-6 pt-0.5 border-t border-gray-300">
                            <div class="text-right flex-1">• تاريخ الاستحقاق: <strong>{{ $dueDateLabel }}</strong></div>
                            <div class="text-left flex-1 text-[10px] text-gray-600" dir="ltr">Due date</div>
                        </div>
                        @endif
                    </div>
                </div>
            ...
            <div class="bg-gray-50 px-6 py-3 border-t-2 border-black text-black">
                <p class="text-[11px] leading-snug text-right">
                    <span class="font-bold">ملاحظات :</span>
                    تم إنشاء وإصدار هذه الفاتورة إلكترونيًا ولا تحتاج لتصديق.
                    شكرًا لثقتك بنا، ونتطلع لتقديم أفضل خدمة لك.
                </p>
                <p class="text-[11px] leading-snug text-left mt-1.5" dir="ltr">
                    <span class="font-bold">Notes :</span>
                    This invoice was created and issued electronically and does not require authentication.
