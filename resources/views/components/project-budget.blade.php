    ...
    <div class="flex justify-between items-center border-b pb-4">
        <div>
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
                إدارة ميزانية المشروع والدفعات
            </p>
        </div>

        {{-- زر تعديل الميزانية (للأدمن فقط) --}}
        ...
        <div
            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-green-700 dark:text-green-300 mt-2 flex items-center gap-1">
        ...
        <div
            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-orange-700 dark:text-orange-300 mt-2 flex items-center gap-1">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
        <div class="p-4 bg-gray-50 dark:bg-gray-700 border-b dark:border-gray-600">
            <h3 class="font-bold text-gray-900 dark:text-white flex items-center gap-2">
                ...
                الدفعات ({{ $SpecialRequest->requestPayments->count() }})
            </h3>
        </div>
        ...
        <div class="divide-y divide-gray-200 dark:divide-gray-700">
            ...
            <div class="p-4 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                <div class="flex items-start justify-between gap-4">
                    <div class="flex-1">
                        ...
                        <div class="flex items-center gap-4 text-sm text-gray-600 dark:text-gray-400">
                            ...
                            <span class="flex items-center gap-1">
                                ...
                                الاستحقاق: {{ \Carbon\Carbon::parse($payment->due_date)->format('Y/m/d') }}
                            </span>
                            @endif
                            @if($payment->paid_at)
                            <span class="flex items-center gap-1 text-green-600">
                                <i class="fas fa-check-circle"></i>
                                دُفعت في: {{ \Carbon\Carbon::parse($payment->paid_at)->format('Y/m/d') }}
                            </span>
                            @endif
                        </div>
                        @if($payment->notes)
                    ...
                    {{-- إذا كانت الدفعة مدفوعة: نعرض زر معاينة الفاتورة فقط --}}
                    @else
                    {{-- @else block --}}
                    <div class="flex flex-col items-end">
                        @php
        ...
        <form action="{{ route('dashboard.special-request.update-budget', $SpecialRequest) }}" method="POST"
            ...
            <div id="installments_section"
                ...
                <div class="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                    <div class="flex justify-between text-sm">
                        ...
                        <span id="total_installments" class="font-bold">0.00</span>
                    </div>
                    <div class="flex justify-between text-sm mt-1">
                        <span>الميزانية:</span>
                        <span id="budget_display" class="font-bold">0.00</span>
                    </div>
            ...
            <div class="flex gap-3 pt-4">
                ...
                <button type="button" onclick="closeEditBudgetModal()"
                    class="flex-1 bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 py-3 rounded-lg font-bold">
                    إلغاء
                </button>
            </div>
        </form>
    ...
    <div class="bg-white dark:bg-gray-800 w-full max-w-md rounded-2xl shadow-2xl overflow-hidden">
        ...
        <form id="paymentProofForm" method="POST" enctype="multipart/form-data" class="p-6 space-y-4">
            ...
            <div class="flex gap-3">
                <button type="submit"
                    class="flex-1 bg-green-600 text-white py-2.5 rounded-lg font-bold hover:bg-green-700">
                    <i class="fas fa-paper-plane ml-1"></i> إرسال
                </button>
                <button type="button" onclick="closePaymentProofModal()"
                    class="flex-1 bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 py-2.5 rounded-lg font-bold">
                    إلغاء
                </button>
            </div>
        </form>
...
// فتح/إغلاق نافذة تعديل الميزانية
function openEditBudgetModal() {
    document.getElementById('editBudgetModal').classList.remove('hidden');
    calculateTotal();
}
...
function addInstallment() {
    ...
    const html = `
        <div class="installment-row flex gap-2 p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
            <input type="text" name="installments[${index}][name]" 
                placeholder="اسم الدفعة" required
                class="flex-1 p-2 rounded border dark:bg-gray-800 dark:text-white text-sm">
