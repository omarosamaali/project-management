...
<div class="p-6 space-y-8">
    ...
    <div class="border-b pb-6">
        ...
        <div class="space-y-5">
            ...
            {{-- نوع النظام / المشروع --}}
            @if($SpecialRequest->project_type)
            <div class="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
    ...
    @php
        ...
        $doneTasks     = $SpecialRequest->tasks->where('status', 'منتهية')->count();
        $inProgTasks   = $SpecialRequest->tasks->where('status', 'قيد الإنجاز')->count();
        $lateTasks     = $SpecialRequest->tasks->where('status', 'متأخرة')->count();
