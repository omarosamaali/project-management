...
$currentTotalShare = $assignedPartners
->filter(fn($p) => $p->pivot->share_type === 'percentage')
...
@endphp
   ...
   <div class="space-y-3">
        ...
        {{-- كارت الشريك --}}
        <div class="bg-gray-50 dark:bg-gray-700 rounded-xl border border-gray-100 dark:border-gray-600 overflow-hidden">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <div class="flex items-center gap-2 flex-wrap">
                            ...
                            @if($SpecialRequest->projectManager && $SpecialRequest->projectManager->user_id == $partner->id)
                            <span
            ...
            <div
                ...
                <div class="p-2 text-center bg-gray-100 dark:bg-gray-700/50 flex flex-col items-center justify-center">
            ...
            @if($tasksCount > 0)
            <div class="divide-y divide-gray-100 dark:divide-gray-700 border-t border-gray-200 dark:border-gray-600">
                ...
                <div
                    ...
                    <div class="flex items-center gap-2">
                        <span class="text-[13px] text-gray-400">{{ $task->start_date }} → {{ $task->end_date }}</span>
                        <span class="text-[13px] font-bold px-1.5 py-0.5 rounded-full {{ $cfg['bg'] }} {{ $cfg['text'] }}">
                            {{ $task->status }}
                        </span>
                    </div>
                </div>
            ...
            <div class="px-3 pb-2">
                <form action="{{ route('dashboard.request.remove-partner', [$SpecialRequest, $partner]) }}"
        ...
        <div class="mb-4">
            ...
            <div class="space-y-3 bg-white dark:bg-gray-800 p-4 rounded-lg border border-gray-300 dark:border-gray-600">
                ...
                <div class="flex items-center justify-between p-2 border-b last:border-b-0">
                    <div class="flex items-center gap-3">
                        <input type="checkbox" id="partner_{{ $partner->id }}" name="partner_id[]" value="{{ $partner->id }}"
                            class="partner-checkbox rounded text-blue-600 focus:ring-blue-500">
                        <label for="partner_{{ $partner->id }}" class="text-sm font-medium text-gray-900 dark:text-white">
                            {{ $partner->name }}
                        </label>
                    </div>
                    <div class="flex items-center gap-2">
                        ...
                        {{-- حقل النسبة --}}
                        <input type="number" id="percentage_{{ $partner->id }}" name="profit_share_percentage[{{ $partner->id }}]"
                            min="0" max="100" placeholder="%" disabled class="percentage-input w-20 bg-gray-50 dark:bg-gray-700 border border-gray-300
                            dark:border-gray-600 text-gray-900 dark:text-white text-sm rounded-lg p-1.5" oninput="validateTotal()">
                        <span class="percentage-label text-gray-600 dark:text-gray-300" data-partner="{{ $partner->id }}">%</span>
                
                        {{-- حقل المبلغ الثابت --}}
                        <input type="number" id="fixed_amount_{{ $partner->id }}" name="fixed_amount[{{ $partner->id }}]" min="0"
                            placeholder="المبلغ" disabled style="display: none;" class="fixed-amount-input w-28 bg-gray-50 dark:bg-gray-700 border border-gray-300
                            dark:border-gray-600 text-gray-900 dark:text-white text-sm rounded-lg p-1.5">
                    </div>
                </div>
                @empty
