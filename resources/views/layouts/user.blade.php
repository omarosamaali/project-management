...
<head>
    ...
    @php
        ...
        $hideSystemsNav = (auth()->check() && (auth()->user()->isTrainee() || auth()->user()->isTrainer() || (auth()->user()->hasAcademyAccess() && $isAcademyPage)))
            ...
            <div class="{{ $isAcademyPage ? 'ac-pub-row'.(auth()->check() ? ' has-auth' : '') : 'flex justify-between h-16' }}">
                ...
                <div class="ac-pub-cluster ac-pub-cluster--start">
                    <div class="ac-pub-side ac-pub-side-start">
                        <div class="ac-pub-links">
                            <a href="{{ route('academy.index') }}" class="ac-pub-cta {{ request()->routeIs('academy.index') || request()->routeIs('academy.home') ? 'is-active' : '' }}">
                                {{ __('messages.home') }}
                            </a>
                ...
                <div class="hidden md:flex items-center gap-2">
                    ...
                    <a href="{{ route('academy.index') }}" class="px-4 py-2 rounded-lg transition
                            {{ request()->routeIs('academy.index') || request()->routeIs('academy.home')
                                ? 'bg-black text-white hover:bg-gray-700'
                                : 'text-gray-700 hover:bg-gray-100' }}">
                        {{ __('messages.home') }}
                    </a>
    ...
    <aside id="mobileMenu"
        ...
        <nav class="flex-1 overflow-y-auto px-3 py-4 space-y-1.5">
            ...
            <a href="{{ route('academy.index') }}"
                class="mobile-drawer-link flex items-center gap-3 px-4 py-3 rounded-xl transition text-sm font-medium {{ request()->routeIs('academy.index') || request()->routeIs('academy.home') ? 'is-drawer-active' : '' }}">
            ...
            <a href="{{ route('academy.index') }}"
                class="mobile-drawer-link flex items-center gap-3 px-4 py-3 rounded-xl transition text-sm font-medium
                    {{ request()->routeIs('academy.index') || request()->routeIs('academy.home') ? 'bg-black text-white' : 'text-gray-700 hover:bg-gray-100' }}">
                <i class="fas fa-home w-5 text-center {{ request()->routeIs('academy.index') || request()->routeIs('academy.home') ? 'text-white/80' : 'text-gray-400' }}"></i>
                {{ __('messages.home') }}
            </a>
            @auth
            @unless(auth()->user()->isTrainee() || auth()->user()->isTrainer())
