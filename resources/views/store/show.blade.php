...
@section('title', 'دورة - ' . ($store->name_ar ?? $store->name_en))
            ...
            <div class="grid md:grid-cols-2 gap-0">
                ...
                <div class="relative h-[500px] md:h-auto md:sticky md:top-0 md:self-start order-1 md:order-none">
                    <div class="h-full md:h-[600px] lg:h-[700px] max-h-[700px]">
                        <img src="{{ asset($store->main_image) }}" alt="{{ $store->name_ar }}"
                ...
                <div class="p-8 order-2 md:order-none overflow-y-auto">
                    ...
                    <h1 class="text-4xl font-bold text-gray-800 mb-4 ltr:text-left rtl:text-right">
                        {{ app()->getLocale() == 'en' ? $store->name_en : $store->name_ar }}
                    </h1>
                    ...
                    <div class="description-wrapper">
                        @php
                        $description =
                        app()->getLocale() == 'en' ? $store->description_en : $store->description_ar;
                        $shortDescription =
                                ...
                                <div class="space-y-1">
                                    <p class="text-gray-700 flex items-center gap-2">
                                        ...
                                        <span class="font-bold">{{
                                            \Carbon\Carbon::parse($store->start_date)->locale('ar')->isoFormat('dddd، D
                                            MMMM YYYY') }}</span>
                                    </p>
                                    <p class="text-gray-700 flex items-center gap-2">
                                        <i class="fas fa-clock text-green-500"></i>
                                        <span class="font-bold">{{
                                            \Carbon\Carbon::parse($store->start_date)->format('h:i A') }}</span>
                                    </p>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="ltr:text-left rtl:text-right flex-1">
                                ...
                                <div class="space-y-1">
                                    <p class="text-gray-700 flex items-center gap-2">
                                        ...
                                        <span class="font-bold">{{
                                            \Carbon\Carbon::parse($store->end_date)->locale('ar')->isoFormat('dddd، D
                                            MMMM YYYY') }}</span>
                                    </p>
                                    <p class="text-gray-700 flex items-center gap-2">
                                        <i class="fas fa-clock text-red-500"></i>
                                        <span class="font-bold">{{ \Carbon\Carbon::parse($store->end_date)->format('h:i
                                            A') }}</span>
                                    </p>
                                </div>
                            </div>
                        ...
                        <div class="flex items-start gap-3 p-4 bg-orange-50 rounded-lg border border-orange-200">
                            ...
                            <div class="ltr:text-left rtl:text-right flex-1">
                                ...
                                <p class="text-gray-700 flex items-center gap-2">
                                    ...
                                    <span class="font-bold">{{
                                        \Carbon\Carbon::parse($store->last_date)->locale('ar')->isoFormat('dddd، D MMMM
                                        YYYY') }}</span>
                                </p>
                            </div>
                        </div>
                    ...
                    <div class="flex items-start gap-3 p-4 bg-gray-100 rounded-lg mb-6">
                        ...
                        <div class="ltr:text-left rtl:text-right flex-1">
                            ...
                            <ul class="space-y-1">
                                @foreach ($store->requirements as $req)
                    ...
                    <!-- Service Badge -->
                    @if ($store->service_id && $store->service)
                    <div
                        ...
                        @if ($store->service->image)
                        <img src="{{ asset('storage/' . $store->service->image) }}"
                            alt="{{ app()->getLocale() == 'ar' ? $store->service->name_ar : $store->service->name_en }}"
                        ...
                        <span class="font-semibold text-sm">
                            {{ app()->getLocale() == 'ar' ? $store->service->name_ar : $store->service->name_en }}
                        </span>
                    </div>
                    @endif

                    <!-- Features -->
                    @if (!empty($store->features))
                    <div class="mb-8">
                    ...
                    <div class="flex flex-col gap-4">
                        @auth
                        @php
                        $current_enrolled = \App\Models\Payment::where('course_id', $store->id)
                        ->where('status', '!=', 'failed')
                        ->count();

                        $actual_remaining = ($store->counter ?? 0) - $current_enrolled;

                        $is_already_in = \App\Models\Payment::where('user_id', auth()->id())
                        ->where('course_id', $store->id)
                        ->exists();
                        @endphp
                        ...
                        <div
                            ...
                            أنت مشترك بالفعل في هذه الخدمة
                        </div>
                        @else
                        {{-- تعديل النوع ليكون store --}}
                        <button onclick="handlePayment({{ $store->id }}, {{ $store->price }}, 'store', 'اشتراك في {{ $store->name_ar }}')"
                    ...
                    @if (!empty($store->buttons))
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 my-8">
                        @foreach ($store->buttons as $button)
...
@if($related_stores && $related_stores->count() > 0)
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-8">
    ...
    <div id="systems-grid" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        @foreach($related_stores as $item)
        <div class="system-card bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2 flex flex-col"
            ...
            <div class="relative h-48 overflow-hidden">
                ...
                <img src="{{ $item->type === 'system' || $item->type === 'service' ? asset($item->main_image) : Storage::url($item->main_image) }}"
...
<script>
    ...
    async function handlePayment(itemId, price, type, title = 'تأكيد الاشتراك') {
        ...
        if (price == 0) {
            const result = await Swal.fire({
                title: '🎉 اشتراك مجاني!',
                text: 'هذه الخدمة مجانية تماماً. هل تريد الاشتراك الآن؟',
                icon: 'success',
            ...
            if (result.isConfirmed) {
                ...
                await proceedPayment(); // سنستخدم دالة واحدة للمعالجة
            }
            return;
        }

        // حساب الرسوم للمدفوع (0.079 + 2)
    ...
    async function proceedPayment() {
        ...
        // تحديد الـ Endpoint والـ Payload بناءً على النوع
        let endpoint = '{{ route("payment.create") }}'; 
        let payload = { type: currentItemType };

        if (currentItemType === 'store') {
            payload.store_id = currentItemId;
        } else if (currentItemType === 'course') {
        ...
        try {
            ...
            if (data.success) {
                if (data.is_free) {
                    await Swal.fire('تم!', data.message, 'success');
